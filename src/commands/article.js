import { fetchFeed, getArticleUrl, formatTimestamp } from '../services/leagueFunnyApi.js'
...
const BOARD_KEYWORDS = new Set()
for (const [bType, info] of Object.entries(BOARD_MAP)) {
  BOARD_KEYWORDS.add(bType)
  BOARD_KEYWORDS.add(info.chi)
  BOARD_KEYWORDS.add(info.eng)
}
...
function isLottoArticle (article) {
  return article.f_type === 'lotto_draw' || article.preview?.lotto_draw != null
}

function buildBubble (article, board) {
  ...
  bubble.footer.contents[0].action.uri = getArticleUrl(board || article.f_game_type, article.fid)

  return bubble
}

export async function handleArticle (event) {
  try {
    ...
    console.log(`[Command] handleArticle | input="${event.message.text}" | board=${board} sort=${sort}`)

    const articles = await fetchFeed(board, sort)

    const bubbles = []
    let count = 0
    for (const article of articles) {
      if (count >= 12) break
      if (article.type === 'ad' || article.type === 'lotto' || article.type === 'kuji') continue
      if (isLottoArticle(article)) continue
